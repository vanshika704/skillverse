...
export default function DashboardPage() {
  return (
    <div className="flex flex-col lg:flex-row w-screen min-h-screen bg-gradient-to-b from-cyan-100 to-white">
      {/* Sidebar hidden on small screens */}
      <div className="hidden lg:block">
        <Sidebar />
      </div>

      <div className="flex flex-col flex-1 px-4 sm:px-6 md:px-12 py-8 space-y-8 max-w-7xl mx-auto">
        <h1 className="text-2xl sm:text-3xl font-bold text-cyan-950">
          Explore Latest Workshops
        </h1>

        <div className="w-full h-[260px] sm:h-[280px] md:h-[300px] rounded-3xl shadow-xl bg-gradient-to-br from-cyan-900 to-white">
          <CoverflowCarousel />
        </div>

        {/* Placeholder for more responsive sections */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6">
          {/* Add responsive artwork or search cards here */}
        </div>
      </div>
    </div>
  );
