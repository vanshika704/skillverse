  ...
  return (
    <div className='flex min-h-screen bg-gray-50'>
      ...
      <div className="flex-1 overflow-auto">
        <header className="bg-gradient-to-r from-cyan-600 to-cyan-500 p-4 md:p-6 shadow-sm">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h1 className="text-2xl md:text-3xl font-bold text-white">Offerskills Dashboard</h1>
        ...
        <main className="max-w-7xl mx-auto p-4 md:p-6">
          <div className="flex flex-wrap gap-1 bg-cyan-50 p-1 rounded-lg mb-6">
            {(['schedule', 'workshops', 'feedback', 'earnings'] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-3 py-2 text-sm font-medium rounded-md transition-all flex-1 min-w-[120px] text-center ${
                  activeTab === tab 
                    ? 'bg-white text-cyan-600 shadow-sm' 
                    : 'text-cyan-700 hover:bg-cyan-100'
                }`}
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            {activeTab === 'feedback' && (
              <div className="p-4 md:p-6">
                <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-cyan-900">Participant Feedback</h2>
                
                <div className="bg-gradient-to-br from-cyan-50 to-white p-4 md:p-6 rounded-xl border border-cyan-100 mb-6 md:mb-8">
                  <div className="flex flex-col sm:flex-row items-center">
                    <div className="text-4xl md:text-5xl font-bold mr-0 sm:mr-6 text-cyan-600 mb-4 sm:mb-0">
                      {feedback.length > 0 
                        ? (feedback.reduce((sum, f) => sum + f.rating, 0) / feedback.length).toFixed(1)
                        : '0.0'}
                    </div>
                    <div className="text-center sm:text-left">
                      <div className="flex justify-center sm:justify-start items-center mb-2">
                        {[...Array(5)].map((_, i) => (
                          <svg 
                            key={i} 
                            className={`w-5 h-5 md:w-6 md:h-6 ${i < Math.round(feedback.reduce((sum, f) => sum + f.rating, 0) / feedback.length) ? 'text-yellow-400' : 'text-gray-300'}`} 
                            fill="currentColor" 
                            viewBox="0 0 20 20"
                          >
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                  ...
                  <select
                    value={feedbackFilter}
                    onChange={(e) => setFeedbackFilter(e.target.value)}
                    className="text-sm border border-cyan-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-cyan-500 w-full sm:w-auto"
                  >
                    <option value="all">All Ratings</option>
                    <option value="high">4-5 Stars</option>
                    <option value="low">1-2 Stars</option>
                ...
                <div className="space-y-4 md:space-y-6">
                  {filteredFeedback.length === 0 ? (
                    ...
                    filteredFeedback.map((item) => (
                      <div key={item.id} className="border-b border-cyan-100 pb-4 md:pb-6 last:border-0">
                        <div className="flex items-start gap-3 md:gap-4">
                          <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-cyan-200 flex items-center justify-center text-cyan-800 font-medium flex-shrink-0">
                            {item.participant.charAt(0)}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1">
                              <div className="min-w-0">
                                <h4 className="font-medium text-cyan-900 truncate">{item.participant}</h4>
                                <div className="flex items-center mt-1">
                                  {[...Array(5)].map((_, j) => (
                                    <svg 
                                      key={j} 
                                      className={`w-3 h-3 md:w-4 md:h-4 ${j < item.rating ? 'text-yellow-400' : 'text-gray-300'}`} 
                                      fill="currentColor" 
                                      viewBox="0 0 20 20"
                                    >
                                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                            ...
                            <div className="mt-2">
                              <span className="text-sm font-medium text-cyan-800 truncate block">Workshop: {item.workshopTitle}</span>
                            </div>
                            <p className="mt-2 md:mt-3 text-cyan-800 text-sm md:text-base">
                              {item.comment}
                            </p>
                          </div>
                        </div>
